...
# supported suffixes
suffixes = ["jpg", "jpeg", "png", "tif", "tiff", "npy"]
...
class ItemList(object):
    def __init__(self, items=[], size=(600, 600)):
...
class AppTk(tk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        self.images = ItemList(items=sorted(set(files)), size=(600, 600))
    ...
    def setup(self):
        ...
        self.lfdata = ttk.Labelframe(self, padding=(2, 2, 4, 80), text="Selection")
        self.lfdata.grid(row=0, column=6, columnspan=2, sticky="ne")
        # for i, item in enumerate(digits + "d"):
        # tree_lsit = ['樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1','樹木1']
        dict = {('針葉樹林型', '其他針'): 1,('針葉樹林型', '冷杉'): 2,('針葉樹林型', '鐵杉'): 3,('針葉樹林型', '檜木（紅檜、扁柏）'): 4,('針葉樹林型', '台灣杉'): 5,('針葉樹林型', '杉木（巒大杉、杉木等）'): 6,('針葉樹林型', '臺灣肖楠'): 7,('針葉樹林型', '柳杉'): 8,('針葉樹林型', '松樹（二葉松、五葉松等）'): 9,('闊葉樹林型', '其他闊'): 10,('闊葉樹林型', '相思樹'): 11,('闊葉樹林型', '大葉桃花心木'): 12,('闊葉樹林型', '台灣赤楊'): 13,('闊葉樹林型', '銀合歡'): 14,('闊葉樹林型', '木麻黃'): 15,('闊葉樹林型', '光臘樹'): 16,('闊葉樹林型', '台灣櫸'): 17,('闊葉樹林型', '木油桐'): 18,('竹林', '單桿狀竹'): 19,('竹林', '叢生狀竹'): 20}
        for i, item in enumerate(dict):
            ttk.Button(
                self.lfdata, text=item, command=partial(self.button_callback, item)
            ).grid(in_=self.lfdata, column=6 + i % 2, row=i // 2, ipady=10, ipadx=10, sticky="w")

    def display(self):
        print(self.images.current)
